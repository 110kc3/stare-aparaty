...
| Privacy policy | `templates/privacy.template.html` → generated `polityka-prywatnosci.html` |
...
Approval odds are better now that `poradniki/` ships five guide pages of
original editorial content — thin content was the main risk with a bare
affiliate catalog. If it's still rejected for "low value content", the fix is
more and deeper guides (per-model, not per-type), not a resubmission.
...
`index.html`, `polityka-prywatnosci.html`, `ads.txt` and `sitemap.xml`.
...
Fill in the administrator identity in `templates/privacy.template.html` §1.
RODO art. 13 requires naming the data controller; the page currently says
"właściciel serwisu" plus the contact email, which is thin if the site is run
under a registered business. Add the business name, address and NIP.
