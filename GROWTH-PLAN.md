...
- **Per-model guides — all five shipped.** Pentax ME, Zenit B + Helios 44-2, and now Praktica BCA, Yashica 35 and Canon EOS. Model-level guides catch the buyer already searching a specific body, which is where intent is highest. They select their listings by keyword rather than by catalog section, so a guide keeps earning after that particular camera sells — and that turned out to matter more than expected: the two Praktica listings sit in *different* catalog sections (one is a Zestaw, one an SLR) and the Canon EOS listing is a Zestaw, so no type guide could ever have gathered them. **The named-candidate list is now empty, and that's deliberate.** Picking a sixth model without data is guessing; do action item 1 below and let impressions choose.
- **Google Search Console**: submit `sitemap.xml` (added), verify the property. Free and takes 10 minutes. The sitemap's `lastmod` dates became per-page and honest on 2026-08-11 — they used to read "today" for all twelve URLs on every nightly build, which is the pattern Google's docs cite as its reason to ignore `lastmod` altogether. A guide that hasn't changed since 7 August now says so, which is what makes the crawl-priority signal usable at all.
