...
- [ ] **"Zarezerwowane" / "Sprzedane" state.** When a camera sells, mark the card visually (desaturated + label) instead of deleting it; gives returning visitors a sense that the site actually moves and avoids dead clicks. Could be a one-field flag in `product-links.txt` (e.g. `https://…olx.pl/… #sold`) parsed by the builder.
