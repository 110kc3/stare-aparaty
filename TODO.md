...
- [x] **Auto-discover listings from the OLX user page.** Instead of pasting links by hand, `scripts/discover-listings.js` reads my OLX user page(s), paginates automatically, keeps only my own active offers whose title matches a camera keyword, and writes `product-links.txt`. *(Done — replaces the old paste-links flow. `KEYWORDS` defaults to `aparat` / `analog` plus common analog-camera brands so model-only titles like "Pentax SF7" aren't dropped.)*
- [x] **Scheduled + on-push rebuild.** `.github/workflows/discover-cameras.yml` runs daily (`30 5 * * *`), on push to the main branches, and manually — discovering, rebuilding, and publishing to Pages in a single run. *(Done — replaces the manual `update-products.yml`, which has been removed. Stale OLX listings drop out automatically and the "Aktualizacja" stamp stays fresh; the delisted-link detector marks sold items.)*
